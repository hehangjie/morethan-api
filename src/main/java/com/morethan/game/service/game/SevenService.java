...
public class SevenService {
    ...
    private static final Map<String, Integer> itemsProbability = new HashMap<String, Integer>() {{
        ...
        put("5:apple-s", 72);
        put("6:mango", 84);
        put("7:watermelon", 102);
        put("8:watermelon-s", 119);
        put("9:lucky-s", 137);
        put("10:apple", 155);
        put("11:orange-s", 172);
        put("12:orange", 190);
    ...
    public Lottery deal(Bet bet) {
        Lottery lottery = new Lottery();
        int r = RandomUtil.rollInt(1, 354);
        for (int i = items.size() - 1; i >= 0; i--) {
            String[] itemArr = items.get(i).split(":");
            lottery.setTitle(itemArr[1]);
            lottery.setType(lottery.getTitle());
            lottery.setItems(new Integer[]{Integer.parseInt(itemArr[0])});
